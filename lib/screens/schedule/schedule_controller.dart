...
class ScheduleController {
  ...
  final slots = ValueNotifier<List<ScheduleSlot>>([]);
  ...
  Future<bool> loadSchedules({DateTime? date}) async {
    ...
    final dateToLoad = date ?? selectedDate.value;
  ...
  Future<bool> addSlot({required DateTime date, required String time}) async {
    ...
    try {
      ...
      if (_isSameDate(newSlot.date, selectedDate.value)) {
        final updatedSlots = List<ScheduleSlot>.from(slots.value)
          ..add(newSlot)
          ..sort(_compareSlots);
      ...
      _errorMessage = 'Unable to add schedule slot.';
  ...
  Future<bool> updateSlot({
    ...
    try {
      ...
      final index = slots.value.indexWhere((item) => item.id == slot.id);
      ...
      if (_isSameDate(updatedSlot.date, selectedDate.value)) {
      ...
      _errorMessage = 'Unable to update schedule.';
  ...
  Future<bool> deleteSlot(ScheduleSlot slot) async {
    ...
    try {
      ...
      _errorMessage = 'Unable to delete schedule slot.';
